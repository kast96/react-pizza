...
import { selectFilterSort, setSort, SortType } from "../redux/slices/filterSlice"
...
export const Sort: FC = () => {
	...
	useEffect(() => {
		const handleClickOutside = (e: MouseEvent & {path?: Array<HTMLDivElement>}) => {
				...
				<div className="sort__popup">
					<ul>
						{sortList.map((item, key) => 
							<li key={item.sortProperty} className={item.sortProperty === sort.sortProperty ? 'active' : ''} onClick={() => onClickListItem(item)}>{item.name}</li>
						)}
					</ul>
				</div>
			}
